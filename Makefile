...
help: ## 显示可用目标与示例
	...
	@printf "  make build\n"
	@printf "  ./bin/db-collector --db-type mysql --db-host 127.0.0.1 --db-port 3306 --db-username root --db-password rootpwd --dbname mysql\n"
	@printf "  GOCACHE=$(GO_CACHE) go run ./collector/cmd/db-collector --db-type mysql --os-only\n"
...
build: build-collector ## 编译客户侧采集器到 bin
	@rm -f "$(TMP_BIN_DIR)/db-reporter" "$(TMP_BIN_DIR)/db-reporter.exe"
