...
func ParseArgs(args []string) (Config, error) {
	...
	if cfg.AWRFile != "" && len(cfg.WDRFiles) != 0 {
	...
	if len(cfg.WDRFiles) != 0 && cfg.RuleFile != "" {
	...
	if len(cfg.WDRFiles) != 0 {
		for i, file := range cfg.WDRFiles {
			cfg.WDRFiles[i] = filepath.Clean(file)
		}
		dbType, err := DetectRunDirDBType(cfg.RunDir)
		if err != nil {
			return Config{}, err
...
常用参数：
  ...
  --wdr-file             GaussDB WDR HTML 报告路径（可重复，仅 GaussDB；与 --rule-file 互斥）
...
func newFlagSet(cfg *Config) *flag.FlagSet {
	...
	fs.Func("wdr-file", "GaussDB WDR HTML 报告路径（可重复，仅 GaussDB）", func(value string) error {
		cfg.WDRFiles = append(cfg.WDRFiles, value)
		return nil
	})
