...
from reporter.content.gaussdb_summary_text import (
    business_dimensions,
...
GAUSSDB_SUMMARY_STRATEGY = SummaryStrategy(
...
def build_gaussdb_report_view(result: dict[str, Any], summary: dict[str, Any], meta: dict[str, Any]) -> ReportView:
    ...
    sections = (
        ...
        build_summary_section(result, summary, meta, GAUSSDB_SUMMARY_STRATEGY),
...
def _resolve_title(meta: dict[str, Any]) -> str:
    ...
    return str(doc_info.get("document_name", "GaussDB巡检报告"))
...
def _build_detail_section(result: dict[str, Any]) -> SectionBlock:
    return SectionBlock(
        ...
        children=(
            ...
            SectionBlock(title="2.2 数据库指标", children=build_gaussdb_database_sections(result)),
        ),
    )
