...
### 1.4 项目范围
- In Scope（v2.0 MVP）
- MySQL 巡检主链路：`db-collector -> db-analyzer -> db-reporter`
- 三层契约与 schema 门禁
- 旧参数兼容映射（优先兼容）
...
3. 运行 `db-analyzer`，生成 `summary.json`。  
...
2. `db-analyzer` 读取同目录产物并写入 `summary.json`。  
...
### 10.2 Analyzer CLI
```bash
db-analyzer --manifest <manifest.json> --result <result.json> --rule <rule.json> --out <summary.json>
```
