...
- 客户视角：只关心 `db-collector` 采集和 Web 上传生成报告
...
从客户使用习惯看，整个客户侧只有一个可执行入口：

1. `db-collector`：连接数据库并采集指标，产出标准 `run` 目录
2. Web 报告页面：上传采集 ZIP 并生成最终 `report.docx`
...
这条约束只针对源码仓库中的 Python 命令。客户使用发布包时只执行 `db-collector`；报告生成统一由 Web 后端管理 Python 运行时。
...
生成 Word 报告：将 `./runs/<run_id>/` 压缩成 ZIP，在 Web 页面上传并下载报告包。
