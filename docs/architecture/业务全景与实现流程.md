...
- 客户视角：只关心 `db-collector` 和 `db-reporter`
- 内部实现视角：关心 contracts、规则、分析、`ReportView` 和模板渲染链路

目标是说明当前 `db-check` 项目如何从一次巡检采集，走到最终 Word 报告交付。

重点回答 4 个问题：

1. 一次巡检从启动到交付最终经历了哪些环节。
2. 每个环节产出什么文件、由谁消费、用什么规范约束。
3. `manifest.json`、`result.json`、`summary.json`、`report-meta.json`、`report-view.json`、`report.docx` 之间是什么关系，以及 `report.md` 为什么只是可选导出物。
...
1. `db-collector`：连接数据库并采集指标，产出标准 `run` 目录
2. `db-reporter`：读取 `run` 目录并生成最终 `report.docx`
...
5. 文档输出层：把 `ReportView` 按 Word 模板渲染成最终 DOCX，并按需导出 Markdown 报告。

客户入口：

- 采集入口：[collector/cmd/db-collector/main.go](/Users/lmj/projects/ai-project/db-check/collector/cmd/db-collector/main.go)
- 报告入口：[reporter/cmd/db-reporter/main.go](/Users/lmj/projects/ai-project/db-check/reporter/cmd/db-reporter/main.go)
...
内部仍然通过分析、元数据、`ReportView` 和模板渲染来完成报告生成：
...
2. `report-view.json` 是正式报告内容契约，`report.md` 是可选的人类可读导出物。
...
`db_report_preview.py` 负责先构建统一 `ReportView`，并输出：

- `report-view.json`（正式内容契约，DOCX 主链路必需）
...
| `report-view.json` | Report Builder | `result + summary + meta` | 统一报告展示模型 | DOCX Renderer / 调试与测试 |
| `report.md` | Markdown Renderer | `ReportView` | 可选 Markdown 导出物 | 人工审阅 / 文本 diff / 调试 |
...
5. `report-view.json` 内容结构正确
6. 如果启用 Markdown 导出，关键章节真的进了 `report.md`
7. 最终 `report.docx` 由模板样式渲染器成功生成
...
### 8.3 Python 必须在虚拟环境中运行（源码开发）

这条约束只针对源码仓库中的 Python 命令。客户使用发布包时，直接调用 `db-reporter` 即可；如果依赖缺失，由启动器显式报错。

源码仓库中的所有 Python 命令必须先激活 `.venv`：
...
```bash
db-reporter --run-dir ./runs/<run_id> --out-md ./runs/<run_id>/report.md
```
...
```bash
go run ./collector/cmd/db-collector \
  --db-type mysql \
  --db-host 127.0.0.1 \
  --db-port 3306 \
  --db-username root \
