...
- `docker/docker-compose.oracle-base.yml`：Oracle 公共环境配置（仅 OS target）
- `docker/docker-compose.oracle11g.yml`：Oracle 11g 覆盖配置
- `docker/docker-compose.oracle19c.yml`：Oracle 19c 覆盖配置（arm64）
...
- `docker/oracle/apply_scenarios.sh`：Oracle 场景初始化（创建 smoke schema / invalid object / disabled constraint / disabled trigger）
...
默认会顺序执行 `mysql` 的 `5.6`、`5.7`、`8.0` 三个版本；也可以按版本指定：
