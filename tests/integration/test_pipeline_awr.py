...
class PipelineAWRIntegrationTests(unittest.TestCase):
    def test_db_reporter_generates_oracle_awr_artifacts_from_run_dir(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            env = dict(os.environ)
            env["GOCACHE"] = "/tmp/go-cache"
            run_dir = temp_path / "oracle-awr-run"
            run_dir.mkdir()

            manifest = _oracle_manifest()
            result = copy.deepcopy(_oracle_result())
            result["meta"]["db_name"] = "ORACC"
            result["db"]["basic_info"]["db_name"] = "ORACC"
            result["db"]["basic_info"]["dbid"] = 2668322570
            ...
            command = [
                ...
                "--awr-file",
                str(ROOT / "resources" / "awrrpt_1_19321_19322.html"),
            ]
            ...
            self.assertIn("4.11", abnormal_ids)
