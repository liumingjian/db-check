...
class TemplateDocxRendererTests(unittest.TestCase):
    def test_template_docx_cli_generates_styled_report(self) -> None:
        ...
        try:
            ...
            alarm_table = self._find_table(document, "风险等级", "定义", "建议响应时效")
            self.assertIsNotNone(alarm_table)
            alarm_widths = [cell.width for cell in alarm_table.rows[0].cells]
            self.assertGreater(alarm_widths[1], alarm_widths[0])
            self.assertGreater(alarm_widths[1], alarm_widths[2])
            health_table = self._find_table(document, "检查维度", "风险等级", "关键发现")
            ...
            self.assertIn('w:color w:val="C00000"', xml)
            self.assertIn('w:color w:val="C69200"', xml)
            self.assertIn('w:color w:val="1565C0"', xml)
            self.assertIn('w:color w:val="2E7D32"', xml)
            all_text = "\n".join(cell.text for table in document.tables for row in table.rows for cell in row.cells)
            self.assertNotIn("风险标识", all_text)
            self.assertNotIn("Apple Color Emoji", xml)
